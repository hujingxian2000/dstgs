...
public:
	...
	// 新增方法用于计算当前Layer对象自身的内存大小（不包含可能存在的派生类额外内存）
    size_t calculateLayerMemorySize() const;
...
// 实现calculateLayerMemorySize方法，计算Layer对象自身内存大小
size_t Layer::calculateLayerMemorySize() const {
	size_t totalSize = 0;
    // 计算basket矩阵的内存大小
    size_t matrixSize = width * depth * sizeof(basket);
	totalSize += matrixSize;
	// 加上基本类型成员变量占用的内存大小（简单累加其类型大小）
    totalSize += sizeof(granularity) + sizeof(width) + sizeof(depth) + sizeof(fingerprintLength)+ sizeof(row_addrs) + sizeof(column_addrs);
    return totalSize ;
}
