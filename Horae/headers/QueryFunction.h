...
int horaeSequentialInsert(HORAE_VAR var, string filename) {
	...
	//计算并输出内存大小
	horae_sequential->updateMemorySizes();
    if (horae_sequential!= nullptr)
    {
        size_t totalMemorySize = horae_sequential->getTotalMemorySize();
...
int horaeParallelInsert(HORAE_VAR var, string filename) {
	...
	//计算并输出内存大小
	horae_parallel->updateMemorySizes();
    if (horae_parallel!= nullptr)
    {
        size_t totalMemorySize = horae_parallel->getTotalMemorySize();
        cout << "插入操作结束后使用的内存大小为: " << totalMemorySize << " 字节" << endl;
    }
    else
    {
