...
// 更新后的updateMemorySizes方法，调用Layer的计算内存大小方法来累加每层内存
void Horae::updateMemorySizes() {
    totalLayerMemorySize = 0;
    for (const auto& layer : multi_layers) {
        totalLayerMemorySize += layer->calculateLayerMemorySize();
    }
	// 加上基本类型成员变量占用的内存大小（简单累加其类型大小）
    totalLayerMemorySize += sizeof(cache_align) + sizeof(kick) + sizeof(row_addrs) + sizeof(column_addrs)+ sizeof(timeslice_len) + sizeof(width) + sizeof(depth) + sizeof(start_time);
}

// 获取totalLayerMemorySize内存大小的方法保持不变
size_t Horae::getTotalMemorySize() const {
    return totalLayerMemorySize;
}
